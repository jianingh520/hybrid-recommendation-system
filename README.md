...
This project implements a hybrid recommendation system that combines item-based collaborative filtering with a gradient boosted decision tree model (XGBoost) to predict user ratings for businesses in the Yelp dataset.

The system is an end-to-end machine learning pipeline built with PySpark for scalable feature extraction across four data sources (users, businesses, check-ins, and photos), producing 35+ engineered features. An item-based CF component captures behavioral similarity between businesses, while XGBoost learns non-linear relationships from structured metadata. The final prediction is a weighted ensemble of both models (α = 0.1).

The hybrid model achieved a hidden test RMSE of **0.9746**, ranking **#10 out of 120+ teams** — outperforming the instructor-provided baselines of 1.09 (CF), 1.00 (XGBoost), and 0.99 (hybrid).

**All code in this repository was written solely by the author, Spencer Hu.**
...
- noise level
- restaurant attire
- parking types (garage, street, lot, valet)
...
- **Log-weighted similarity**: Each similarity score is scaled by `log(1 + common_users) / log(1 + 5)` to down-weight low-confidence similarities computed from few co-raters.
- **Confidence scaling**: The predicted deviation is dampened by `min(1.0, log(1 + business_support) / log(1 + 50))`, preventing over-correction for businesses with sparse ratings.
- **Cold-start fallbacks**: Three separate strategies handle unseen entities:
  - New business, known user → 70% user average + 30% global average
  - New user, known business → 70% business average + 30% global average
  - Both unseen → global average

### 2. Gradient Boosted Model

The model-based component uses a gradient boosted decision tree model implemented with XGBoost. It is trained on engineered user, business, and interaction features derived from the Yelp dataset.

This model captures non-linear relationships between users and businesses that collaborative filtering alone cannot represent. Gradient boosted trees are particularly effective for this task because they can handle heterogeneous feature types and learn complex feature interactions from structured data.
...
The blending weight α was selected based on validation performance. Empirical experiments showed that α = 0.1 produced the lowest RMSE, with XGBoost dominating (90%) because structured features generalize better, while CF contributes signal for known user–item pairs.
...
| Model | RMSE |
|---|---|
| CF baseline (instructor) | 1.0900 |
| XGBoost baseline (instructor) | 1.0000 |
| Hybrid baseline (instructor) | 0.9900 |
| **My Hybrid — validation set** | **0.9767** |
| **My Hybrid — hidden test set** | **0.9746** |

The final system achieved a hidden test RMSE of **0.9746**, ranking **#10 out of 120+ teams**. The test RMSE being slightly better than validation (0.9746 vs 0.9767) suggests the model is not overfitting and generalizes cleanly across both sets.
...
- **Online & implicit feedback**: Incorporate watch time, clicks, and skips as implicit signals; move toward streaming model updates.
